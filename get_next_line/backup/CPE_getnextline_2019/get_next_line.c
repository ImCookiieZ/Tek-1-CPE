...
int copy_tmp(char *buffer_ret, char *left_readed, int func)
{
    ...
    if (tmp[i] == '\n') {
        ret = 0;
        for (j = 0; tmp[i] != '\0'; left_readed[j] = tmp[i], i++, j++);
    }
    left_readed[j] = 0;
    free(tmp);
    return (ret);
...
*left_readed)
{
    for (; vars->ret != 0 && vars->ret != -1; vars->counter++) {
        ...
        buffer_ret[vars->k] = 0;
        if (vars->i != vars->ret) {
            for (vars->j = 0; vars->i < vars->ret; vars->i++, vars->j++)
...
char *get_next_line(int fd)
{
    ...
    vars_t vars = {1, 0, 0, 0, 0, 0, NULL, ret, fd, new_line};

    buffer_ret[0] = 0;
    if (left_readed == NULL)
        left_readed = malloc(sizeof(char) * (READ_SIZE * 50 + 1));
    if ((ret == 0 || ret == -1) && left_readed[0] == 0 || READ_SIZE <= 0) {
        return (NULL);
    }
